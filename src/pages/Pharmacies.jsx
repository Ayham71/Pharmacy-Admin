...
const Pharmacies = () => {
  ...
  const fetchPharmacies = async () => {
    try {
      ...
      const normalized = pharmaciesList.map((p) => {
        const id = p.id || p.Id || p.ID || p.pharmacyId || p.PharmacyId || p.userId || p.UserId || Math.random()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok && response.status !== 204) {
        ...
        setError(`Failed to update pharmacy (ID: ${editingId}): ${errorMessage}`)
  ...
  const handleAddPharmacy = async () => {
    ...
    try {
      ...
      formData.append('Latitude', newPharmacy.latitude ? parseFloat(newPharmacy.latitude) : 0)
      formData.append('Longitude', newPharmacy.longitude ? parseFloat(newPharmacy.longitude) : 0)

      if (newPharmacy.image) {
        formData.append('image', newPharmacy.image)
      }
      ...
      if (newPharmacy.imagePreview) {
        setPharmacyImages(prev => ({
          ...prev,
          [newId]: newPharmacy.imagePreview
        }))
      }
